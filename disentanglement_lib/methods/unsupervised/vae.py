...
@gin.configurable("vae_bounding")
class BetaVAE_bounding(BaseVAE):
    """BetaVAE model."""
    ...
    """
        self.beta = beta
        super(BetaVAE_bounding, self).__init__(bounding=True)
...
@gin.configurable("factor_vae_bounding")
class FactorVAE_Bounding(BaseVAE):
    """FactorVAE model."""
    ...
    def model_fn(self, features, labels, mode, params):
        """TPUEstimator compatible model function."""
        bounding_boxes = get_bounding_boxes(features)
        is_training = (mode == tf.estimator.ModeKeys.TRAIN)
        data_shape = features.get_shape().as_list()[1:]
        z_mean, z_logvar = self.gaussian_encoder(features, is_training=is_training)
        z_sampled = self.sample_from_latent_distribution(z_mean, z_logvar)
        ...
        per_sample_loss_1 = tf.convert_to_tensor(100., dtype=tf.float32)*losses.make_reconstruction_loss_bounding(features*bounding_boxes, reconstructions*bounding_boxes)
        reconstruction_loss_1 = tf.reduce_mean(per_sample_loss_1)
        factor_vae_loss = tf.add(factor_vae_loss, reconstruction_loss_1, name="loss")
